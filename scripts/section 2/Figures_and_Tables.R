...
## This script generates Tables 1,2,3 and Figures 1,2,3
## in sections 2 of the paper
...
# Meta Data
cols    <- c('fixture_id', 'team1', 'team1_id', 'team2', 'team2_id')
team.df <-
  as.data.table(read.csv(paste0(dir_path, "fixture_info_engpr.csv"))[, cols])

# Data Import
data <- load_data(paste0(dir_path, "season1"))

# Basic cleaning

# Remove event types which do not have associated location
data <-
  data[-which(
    data$type %in% c(
      "PenaltyFaced",
...
data <-
  data[-which(
    data$type %in% c("CornerAwarded", "Foul", "Aerial", "Challenge") &
...
snapData <- head(data[data$match_id == 1483492, cols], n = 9)
...
teams_s1 <-
  data.table(data.frame(team_id = 1:20, team_name = sort(unique(team.df$team1[1:380]))))
...
agg.df <-
  aggregate.data.frame(
    x = data.frame(Frequency = dataset$type),
    by = list(Event = dataset$type),
    FUN = length
  )
...
pitch <- data.frame(
  x1 = c(0, 0, 0, 100, 50, 0, 16.5, 16.5, 100, 83.5, 83.5),
...
rowdf      <- dataset[dataset$match_id == '1483527',]
rowdf      <- rowdf[316:328,]
rowdf$time <- 60 * rowdf$expanded_minute + rowdf$second
df         <- data.frame(
  x = NA,
  y = NA,
  team = NA,
...
for (i in 1:nrow(rowdf)) {
  if (i > 1) {
    if (rowdf[i, "team_id"] != rowdf[i - 1, "team_id"]) {
      row           <- data.frame(NA, NA, rowdf[i, "team_id"], NA, NA)
      colnames(row) <- colnames(df)
      df            <- rbind(df, row)
    }
  }
  
  row1      <- rowdf[i, c("x", "y", "team_id", "type", "time")]
  row2      <-
    rowdf[i, c("end_x", "end_y", "team_id", "type", "time")]
  row2$type <- 'Touch'
...
highcol    <- 'red'
lowcol.hex <-
  as.hexmode(round(col2rgb(highcol) * amin + 255 * (1 - amin)))
lowcol     <-
  paste0("#", sep = "", paste(format(lowcol.hex, width = 2), collapse = ""))
...
my.labs <-  function(breaks) {
  c('low', rep('', length(breaks) - 2) , 'high')
}
...
my.heatmap <- function(heat.data, my.title = '') {
  ...
  ggplot(df, aes(x, y)) +
    ...
    theme_bw() + scale_y_continuous(breaks = NULL, expand = c(0, 0)) + scale_x_continuous(breaks =
                                                                                            NULL, expand = c(0, 0)) +
...
p1 <-
  my.heatmap(data[which(data$team_id %in% teams[1] &
                          data$match_id %in% team.df$fixture_id[team.df$team1_id == teams[1]]),])
p2 <-
  my.heatmap(data[which(data$team_id %in% teams[1] &
                          data$match_id %in% team.df$fixture_id[team.df$team2_id == teams[1]]),])
p3 <-
  my.heatmap(data[which(data$team_id %in% teams[2] &
                          data$match_id %in% team.df$fixture_id[team.df$team1_id == teams[2]]),])
p4 <-
  my.heatmap(data[which(data$team_id %in% teams[2] &
                          data$match_id %in% team.df$fixture_id[team.df$team2_id == teams[2]]),])
